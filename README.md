  ...
  7. 赠送指令：
   ...
   8. 打工指令：
      ...
      - 地点关键词如下：
         ...
         | 拉面馆 | wutagawa_laamen | wtgw, wutagawa, 拉面, 拉面馆, lm, lmg |
         | 温泉旅馆 | rokka_onsenryokan | 温泉, 温泉旅馆, wq, wqlg, rokka, 六花, lock |
         | 游乐园 | Tsurumaki_amusement_park | 弦卷, trmk, 游乐园, yly, tsurumaki |
      ...
      - 如不填写时长则默认工作时长为3小时，工作结束后需要手动结束工作。

      -  工资基础为2资金一小时，最多3人同时工作；
         各成员按工作地点和稀有度获得工资加成；
         在连续工作超过3小时后工资减半。
         稀有度为4，5，6的角色分别能获得1.5, 2和3倍的工资加成。
         具体工作地点加成如下：

      | 店铺名称 | popipa | afterglow | roselia | hello_happy_world | pastel_palettes | morfonica | RAS | mygo | ave mujica | toge | kessoku band |
      |---------|--------|-----------|---------|-------------------|-----------------|-----------|-----|------|------------|------|--------------|
   ...
   9. 下班指令
      - 指令`下班`或`xb`可以使工作结束的卡牌回归休息状态。


   10. 工作状态指令
      - 指令 `工作状态`/`gzzt`或`gzt`可以查询以用户当前工作和休息中的卡牌。

   11. 卡牌工作状态指令
      - 指令`卡牌工作状态 [卡牌id]`/`kpgzzt[卡牌id]`或`kgzt[卡牌id]`
        可以查看当前用户特定的卡牌id的工作状态。
   ...
   12. 停止工作指令
      - 使用`停止工作/tzgz/tg [卡牌id]`可以放弃工资，使正在工作的卡牌回归空闲状态。

   13. 抽签指令
      - 使用`抽签`或`cq`指令可以使用30资金抽签，获得随机资金。
...
## 开发者文档
   ...
   > AI 仅参与文档表达优化、中英文一致性处理与css美化，不参与系统设计与功能实现。
